...
| [`vcb_utils.parse_common_run_args`](src/vcb/vcb_utils.py#L47) | `--processor --region --nano-version --fatjet-pt-cut --fatjet-bb-preselection --prescale-factor` |
| [`vcb/run.py`](src/vcb/run.py#L382) | `--skimmer --naming-tag --outdir --write-final-weight --root-only --output-root-location` |

Boolean flags are mutually exclusive pairs: `add_bool_arg` generates a `--no-<name>`
alongside each of `--save-root`, `--save-systematics` and `--fatjet-bb-preselection`,
and `--write-final-weight` uses argparse's own `BooleanOptionalAction`. `--root-only`
is a plain `store_true` with no negation.
...
| `--write-final-weight` / `--no-write-final-weight` | **on** | Add `finalWeight = weight / np_nominal` to the parquet and ROOT. Condor passes `--no-`, then normalizes globally. |
...
**`--root-only` cancels `--write-final-weight`.** `finalWeight` is added by
rewriting the parquet batches, which `--root-only` does not produce. Since
`--write-final-weight` defaults *on*, this fires on every plain `--root-only`
run; it prints a line rather than failing.
...
| Flag | Why |
|---|---|
| `--fatjet-pt-cut` | Stored as `self._fatjet_pt_cut`, never read. |
| `--fatjet-bb-preselection` | Stored as `self._fatjet_bb_preselection`, never read. |
...
python -m vcb.run --processor skimmer --skimmer vcbSkimmer --year 2024 \
  ...
  --outdir <work> --no-write-final-weight
```
