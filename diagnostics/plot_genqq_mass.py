...
def plot_genqq_invariant_mass(
    input_file: str | Path,
    output_pdf: str | Path | None = None,
    tree_name: str = DEFAULT_TREE,
) -> dict[str, float | str]:
    """
    Plot m(GenQ1 + GenQ2) from a Calib_ChargeTagger skimmed ROOT file.
    ...
    if output_pdf is None:
        output_path = input_path.with_name(f"{input_path.stem}_genqq_mass.pdf")
    else:
    ...
    masses = np.asarray((genq1 + genq2).mass, dtype=np.float64)
    ...
    ax.set_xlabel(r"$m(\mathrm{GenQ1}+\mathrm{GenQ2})$ [GeV]")
    ax.set_ylabel("Normalized events / GeV")
    ax.set_title("Invariant mass of GenQ1 + GenQ2")
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "from a Calib_ChargeTagger skim ROOT file."
        )
    )
...
if __name__ == "__main__":
    args = _parse_args()
    result = plot_genqq_invariant_mass(
