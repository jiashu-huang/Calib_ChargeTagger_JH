...
def _grid_search_gaussian_fit(
    ...
    best = None
    mu_min = max(HIST_RANGE[0], peak_center - 20.0)
    mu_max = min(HIST_RANGE[1], peak_center + 20.0)
...
def _fit_gaussian_to_histogram(
    ...
    fit_mask = (bin_centers >= peak_center - 20.0) & (bin_centers <= peak_center + 20.0)
    ...
    if curve_fit is not None:
        params, _ = curve_fit(
            ...
            bounds=([0.0, HIST_RANGE[0], 0.5], [np.inf, HIST_RANGE[1], 40.0]),
            maxfev=20000,
        )
...
def plot_genqqb_invariant_mass(
    input_file: str | Path,
    output_pdf: str | Path | None = None,
    tree_name: str = DEFAULT_TREE,
) -> dict[str, float | str]:
    """
    Plot m(GenQ1 + GenQ2 + GenHadTopB) from a Calib_ChargeTagger skimmed ROOT file.

    The histogram uses 1 GeV bins over [120, 220] GeV, is normalized to unit area
    over the plotted range, and is fit with a Gaussian around the central peak.
    """

    input_path = Path(input_file).expanduser().resolve()
    if output_pdf is None:
        output_path = input_path.with_name(f"{input_path.stem}_genqqb_mass.pdf")
    else:
    ...
    genq2 = vector.array(
        {
            ...
            "eta": arrays["GenQ2Eta"],
            "phi": arrays["GenQ2Phi"],
            "mass": arrays["GenQ2Mass"],
        }
    )
    genhadtopb = vector.array(
        {
            "pt": arrays["GenHadTopBPt"],
            "eta": arrays["GenHadTopBEta"],
            "phi": arrays["GenHadTopBPhi"],
            "mass": arrays["GenHadTopBMass"],
        }
    )

    masses = np.asarray((genq1 + genq2 + genhadtopb).mass, dtype=np.float64)
    ...
    ax.set_xlabel(r"$m(\mathrm{GenQ1}+\mathrm{GenQ2}+\mathrm{GenHadTopB})$ [GeV]")
    ax.set_ylabel("Normalized events / GeV")
    ax.set_title("Invariant mass of GenQ1 + GenQ2 + GenHadTopB")
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Plot the normalized invariant-mass distribution of GenQ1 + GenQ2 + "
            "GenHadTopB from a Calib_ChargeTagger skim ROOT file."
        )
    )
...
if __name__ == "__main__":
    args = _parse_args()
    result = plot_genqqb_invariant_mass(
